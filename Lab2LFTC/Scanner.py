...
class Scanner:
    ...
    def run_program(self):
        ...
        filename = "p1err"
        error_message = ""
        fa_constant = FiniteAutoma.FiniteAutomata.readFromFile('fa-constant.in')
        fa_identifier = FiniteAutoma.FiniteAutomata.readFromFile('fa-identifier.in')
        with open(filename, 'r') as file:
            ...
            for line in file:
                ...
                for i in range(len(tokens)):
                    ...
                    elif fa_identifier.isAccepted(tokens[i]):
                        id = self.symbol_table.add(tokens[i])
                        self.pif.add("id", id)
                    elif fa_constant.isAccepted(tokens[i]):
